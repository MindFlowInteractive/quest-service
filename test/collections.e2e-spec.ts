...
describe('Collections E2E', () => {
    ...
    const moduleRef = await Test.createTestingModule({
      imports: [
        TypeOrmModule.forRoot({
          ...
          dropSchema: true,
          entities: [
            __dirname + '/../src/collections/entities/*.ts',
            __dirname + '/../src/collections/entities/*.js',
          ],
          synchronize: true,
          logging: false,
        }),
  ...
  it('should create category and collection, assign puzzle, track progress and give reward once', async () => {
    // create category
    const resCat = await ds.manager.insert('category', {
      name: 'Logic',
      slug: 'logic',
    });
    ...
    await request(app.getHttpServer())
      .post(`/collections/${collection.id}/assign`)
      .send({ puzzle_id: puzzle1, order_index: 1 });
    await request(app.getHttpServer())
      .post(`/collections/${collection.id}/assign`)
      .send({ puzzle_id: puzzle2, order_index: 2 });

    // user completes first puzzle
    const user = 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa';
    const res1 = await request(app.getHttpServer())
      .post('/collections/complete')
      .send({ user_id: user, puzzle_id: puzzle1 });
    ...
    const res2 = await request(app.getHttpServer())
      .post('/collections/complete')
      .send({ user_id: user, puzzle_id: puzzle2 });
    ...
    const rewards = await ds.manager.query(
    ...
    const rewards2 = await ds.manager.query(
      'SELECT * FROM user_rewards WHERE user_id = $1',
      [user],
    );
