...
export class DatabaseConfigService {
  ...
  public getConfig(): DatabaseConfig {
    ...
    return {
      ...
      username: isTest ? (process.env.TEST_DB_USER || 'postgres') : (process.env.DB_USER || 'postgres'),
      password: isTest ? (process.env.TEST_DB_PASSWORD || 'password') : (process.env.DB_PASSWORD || 'password'),
      database: isTest ? (process.env.TEST_DB_NAME || 'myapp_test') : (process.env.DB_NAME || 'myapp'),
  ...
  public getTypeOrmConfig(): DataSourceOptions {
    ...
    return {
      ...
      synchronize: false,
      logging: config.logging ? ['query', 'error', 'schema', 'warn', 'info', 'log'] : false,
