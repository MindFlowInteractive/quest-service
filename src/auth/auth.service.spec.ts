...
describe('AuthService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        },
        {
          ...
          useFactory: mockBackupCodeRepository,
        },
    ...
    backupCodeRepository = module.get<Repository<TwoFactorBackupCode>>(
      getRepositoryToken(TwoFactorBackupCode),
    );
    ...
    jest
      ...
      .mockImplementation((password: string, hash: string) =>
        Promise.resolve(`hashed_${password}` === hash),
      );
  });
    ...
    it('should successfully register a user', async () => {
      ...
      expect(userRepository.save).toHaveBeenCalledWith(
        expect.objectContaining({
          email: registerDto.email,
          password: `hashed_${registerDto.password}`,
        }),
      );
    });
  ...
  describe('refreshToken', () => {
    it('should generate new tokens if refresh token is valid', async () => {
      ...
      jest
        .spyOn(refreshTokenRepository, 'save')
        .mockResolvedValue({ ...mockExistingToken, isRevoked: true });
      jest
        .spyOn(refreshTokenRepository, 'create')
        .mockReturnValue({} as RefreshToken); // For new token creation
    ...
    it('should throw UnauthorizedException if refresh token is invalid or expired', async () => {
      ...
      await expect(
        service.refreshToken(userId, oldRefreshToken),
      ...
      jest
        .spyOn(service, 'revokeAllRefreshTokensForUser')
        .mockResolvedValue(undefined); // Mock this to prevent actual DB call
      await expect(
        service.refreshToken(userId, 'expired-token'),
      ).rejects.toThrow(UnauthorizedException);
      expect(service.revokeAllRefreshTokensForUser).toHaveBeenCalledWith(
        userId,
      );
    });
  });
  ...
  describe('logout', () => {
    it('should revoke the refresh token and log out', async () => {
      ...
      jest
        .spyOn(refreshTokenRepository, 'save')
        .mockResolvedValue({ ...mockToken, isRevoked: true });
  ...
  describe('validateRefreshToken', () => {
    ...
    it('should return false if refresh token is invalid, revoked, or expired', async () => {
      jest
        ...
        .mockResolvedValue(undefined);
      expect(
        await service.validateRefreshToken('uuid-1', 'invalid-token'),
      ).toBe(false);
      ...
      expect(
        await service.validateRefreshToken('uuid-1', 'revoked-token'),
      ).toBe(false);
      ...
      jest
        ...
        .mockResolvedValue(expiredToken);
      expect(
        await service.validateRefreshToken('uuid-1', 'expired-token'),
      ).toBe(false);
    });
  });
  ...
  describe('findOrCreateOAuthUser', () => {
    ...
    it('should throw error if default user role not found when creating new user', async () => {
      ...
      jest
        ...
        .mockResolvedValueOnce(undefined) // No user by githubId
        .mockResolvedValueOnce(undefined); // No user by email
      jest.spyOn(roleRepository, 'findOne').mockResolvedValue(undefined); // No default role

      await expect(
        service.findOrCreateOAuthUser('github', oauthUser),
      ).rejects.toThrow('Default user role not found. Please seed roles.');
    });
  });
  ...
  describe('Two-Factor Authentication', () => {
    describe('generateTwoFactorSecret', () => {
      ...
      it('should throw error if user not found', async () => {
        ...
        await expect(
          service.generateTwoFactorSecret('invalid-id'),
        ).rejects.toThrow(UnauthorizedException);
      });
      ...
      it('should throw error if 2FA already enabled', async () => {
        ...
        jest.spyOn(userRepository, 'findOne').mockResolvedValue(mockUser);
        await expect(service.generateTwoFactorSecret('uuid-1')).rejects.toThrow(
          BadRequestException,
        );
      });
    });
    ...
    describe('verifyTwoFactorSetup', () => {
      it('should enable 2FA and return backup codes on valid code', async () => {
        ...
        jest
          ...
          .mockResolvedValue({ ...mockUser, isTwoFactorEnabled: true });
        jest
          .spyOn(backupCodeRepository, 'create')
          .mockReturnValue({} as TwoFactorBackupCode);
        jest
          .spyOn(backupCodeRepository, 'save')
          .mockResolvedValue({} as TwoFactorBackupCode);
      ...
      it('should throw error if secret not generated', async () => {
        ...
        jest.spyOn(userRepository, 'findOne').mockResolvedValue(mockUser);
        await expect(
          service.verifyTwoFactorSetup('uuid-1', '123456'),
        ).rejects.toThrow(BadRequestException);
      });
    });
    ...
    describe('disableTwoFactor', () => {
      it('should disable 2FA with valid code and password', async () => {
        ...
        jest
          ...
          .mockImplementation(() => Promise.resolve(true));
        jest
          .spyOn(backupCodeRepository, 'update')
          .mockResolvedValue({ affected: 8 } as any);
      ...
      it('should throw error if 2FA not enabled', async () => {
        ...
        jest.spyOn(userRepository, 'findOne').mockResolvedValue(mockUser);
        await expect(
          service.disableTwoFactor('uuid-1', '123456', 'password123'),
        ).rejects.toThrow(BadRequestException);
      });
    });
    ...
    describe('challengeTwoFactor', () => {
      it('should exchange mfa_pending token for full tokens', async () => {
        ...
        const result = await service.challengeTwoFactor(
          'mfa-pending-token',
          '123456',
        );
      ...
      it('should throw error if mfa_pending token is invalid', async () => {
        ...
        await expect(
          service.challengeTwoFactor('invalid-token', '123456'),
        ).rejects.toThrow();
      });
    });
    ...
    describe('getTwoFactorStatus', () => {
      ...
      it('should throw error if user not found', async () => {
        jest.spyOn(userRepository, 'findOne').mockResolvedValue(null);
        await expect(service.getTwoFactorStatus('invalid-id')).rejects.toThrow(
          UnauthorizedException,
        );
      });
    });
