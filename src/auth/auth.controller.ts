...
export class AuthController {
  ...
  @ApiBody({
    type: VerifyEmailDto,
    examples: {
      'Verify Email': { value: { token: 'your-verification-token' } },
    },
  })
  @HttpCode(HttpStatus.OK)
  ...
  @ApiBody({
    ...
    examples: { 'Forgot Password': { value: { email: 'test@example.com' } } },
  })
  @HttpCode(HttpStatus.OK)
  ...
  @ApiBody({
    ...
    examples: {
      'Reset Password': {
        value: { token: 'your-reset-token', newPassword: 'newPassword123' },
      },
    },
  })
  @HttpCode(HttpStatus.OK)
  ...
  @ApiResponse({
    ...
    schema: {
      example: { userId: 'uuid', email: 'test@example.com', roles: ['user'] },
    },
  })
  ...
  @ApiResponse({
    ...
    schema: {
      example: {
        message: 'Welcome, Admin test@example.com! This is sensitive data.',
      },
    },
  })
  @ApiResponse({ status: 401, description: 'Unauthorized.' })
  ...
  getAdminData(req: RequestWithUser) {
    return {
      message: `Welcome, Admin ${req.user.email}! This is sensitive data.`,
    };
  }
  ...
  @ApiResponse({
    ...
    schema: {
      example: {
        ...
        otpauthUrl:
          'otpauth://totp/Quest%20Service:user@example.com?secret=JBSWY3DPEHPK3PXP&issuer=Quest%20Service',
  ...
  async verifyTwoFactor(
    ...
    const result = await this.authService.verifyTwoFactorSetup(
      req.user.id,
      verifyDto.code,
    );
    return result;
  ...
  @ApiOperation({
    summary:
      'Challenge endpoint to exchange mfa_pending token for full JWT tokens',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({
    status: 401,
    description: 'Invalid MFA pending token or 2FA code.',
  })
  @ApiBody({
    schema: {
      type: 'object',
  ...
  async challengeTwoFactor(
    @Body() challengeDto: ChallengeTwoFactorDto & { mfaPendingToken: string },
  ) {
    const result = await this.authService.challengeTwoFactor(
      challengeDto.mfaPendingToken,
      challengeDto.code,
    );
    return result;
  }
  ...
  @Post('2fa/disable')
  @ApiOperation({
    summary: 'Disable 2FA (requires current TOTP code + password)',
  })
  @ApiBearerAuth()
