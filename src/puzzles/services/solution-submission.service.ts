...
export class SolutionSubmissionService {
  ...
  async submitSolution(
    ...
    if (isCorrect && !fraudResult.isFraud) {
      rewardData = this.calculateRewards(
        puzzle,
        timeTakenSeconds,
        dto.hintsUsed ?? 0,
      );
  ...
  private async enforceRateLimit(userId: string): Promise<void> {
    ...
    if (recentCount >= RATE_LIMIT_MAX) {
      this.logger.warn(
        `Rate limit hit for user ${userId}: ${recentCount} submissions in last 60s`,
      );
  ...
  private async detectFraud(
    ...
    const minExpectedSeconds =
      difficultyRating * MIN_SECONDS_PER_DIFFICULTY_UNIT;
  ...
  private calculateRewards(
    ...
    if (puzzle.timeLimit && puzzle.timeLimit > 0) {
      ...
      if (timeRemaining > 0) {
        timeBonus = Math.round(
          (timeRemaining / puzzle.timeLimit) * baseScore * 0.5,
        );
      }
    }
    ...
    const totalScore = Math.max(
      0,
      baseScore + timeBonus + streakBonus - hintPenalty + firstSolveBonus,
    );
    const totalExperience =
      Math.round(totalScore * 0.5) + achievements.length * 50;
  ...
  private determineAchievements(
    ...
    if (
      hintsUsed === 0 &&
      puzzle.timeLimit &&
      timeTakenSeconds < puzzle.timeLimit * 0.5
    ) {
