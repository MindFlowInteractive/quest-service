...
export class PuzzleReviewService {
  ...
  async submitReview(
    ...
    const moderationStatus = this.checkProfanity(createReviewDto.reviewText)
      ? 'flagged'
      : 'pending';
  ...
  async getPuzzleReviews(
    puzzleId: string,
    page: number = 1,
    limit: number = 20,
    sort: 'recency' | 'helpful' = 'recency',
  ): Promise<{ reviews: PuzzleReview[]; total: number }> {
    const query = this.reviewRepository
      .createQueryBuilder('review')
      .where('review.puzzleId = :puzzleId', { puzzleId })
      .andWhere('review.moderationStatus IN (:...statuses)', {
        statuses: ['approved', 'pending'],
      }) // Only show approved or pending
