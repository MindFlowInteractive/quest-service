...
function makeDto(
  overrides: Partial<SubmitSolutionDto> = {},
): SubmitSolutionDto {
  return {
  ...
  return {
    ...
    save: jest
      ...
      .mockImplementation((entity) =>
        Promise.resolve({ id: 'attempt-id-001', ...entity }),
      ),
  };
}

function makeDataSource(saved?: Partial<PuzzleSolutionAttempt>) {
  const savedAttempt = saved ?? {
    id: 'attempt-id-001',
    status: SolutionAttemptStatus.CORRECT,
  };
  return {
...
describe('SolutionSubmissionService', () => {
  ...
  async function buildService(
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: getRepositoryToken(PuzzleSolutionAttempt),
  ...
  describe('✅ Correct answer submission', () => {
    ...
    it('awards speed_demon achievement when solved in < 30% of time limit', async () => {
      ...
      const result = await service.submitSolution(
        'user-1',
        'puzzle-uuid-001',
        makeDto({ sessionStartedAt: makeSessionStart(80) }),
      );
  ...
  describe('🔒 Anti-replay: duplicate nonce', () => {
    it('throws ConflictException when nonce already used', async () => {
      ...
      await buildService(basePuzzle, {
        id: 'old-attempt',
        nonce: usedNonce,
      } as PuzzleSolutionAttempt);

      await expect(
        service.submitSolution(
          'user-1',
          'puzzle-uuid-001',
          makeDto({ nonce: usedNonce }),
        ),
  ...
  describe('⏰ Time limit validation', () => {
    ...
    it('increments attempts even on timeout', async () => {
      ...
      expect(puzzleRepo.increment).toHaveBeenCalledWith(
        { id: 'puzzle-uuid-001' },
        'attempts',
        1,
      );
  ...
  describe('💡 Hint penalty scoring', () => {
    ...
    it('reduces score when hints are used', async () => {
      const noHints = await service.submitSolution(
        'user-1',
        'puzzle-uuid-001',
        makeDto({ hintsUsed: 0 }),
      );
      const withHints = await service.submitSolution(
        'user-1',
        'puzzle-uuid-001',
        makeDto({ hintsUsed: 2 }),
      );

      expect(withHints.rewards.totalScore).toBeLessThan(
        noHints.rewards.totalScore,
      );
    });
