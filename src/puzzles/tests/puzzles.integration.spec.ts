...
describe('Puzzles Integration Test', () => {
  ...
  beforeAll(async () => {
    module = await Test.createTestingModule({
      ...
      providers: [PuzzlesService, PuzzleRatingService, PuzzleReviewService],
    }).compile();
  ...
  describe('Rating System Integration', () => {
    it('should correctly aggregate ratings when multiple users rate a puzzle', async () => {
      ...
      const updatedPuzzle = await puzzlesService.findOne(
        puzzle.id,
        'user-creator-id',
      );
    ...
    it('should prevent duplicate reviews from the same user', async () => {
      ...
      await reviewService.submitReview('user-1', puzzle.id, {
        reviewText: 'First review',
      });

      await expect(
        reviewService.submitReview('user-1', puzzle.id, {
