...
export class InvalidationService {
  ...
  async invalidateByRule(ruleName: string): Promise<void> {
    ...
    try {
      ...
      if (rule.pattern) {
        ...
        for (const key of keys) {
          await this.cacheService.delete(
            key.replace(process.env.CACHE_KEY_PREFIX || 'app:', ''),
          );
        }
      }
      ...
      this.logger.error(
        `Error invalidating cache with rule ${ruleName}:`,
        error,
      );
      throw error;
    }
  ...
  async invalidateByEntity(
    ...
    try {
      ...
      this.logger.log(
        `Cache invalidated for entity: ${entityName}${
          entityId ? `:${entityId}` : ''
        }`,
      );
    } catch (error) {
      this.logger.error(
        `Error invalidating cache for entity ${entityName}:`,
        error,
      );
      throw error;
    }
