  ...
  (): CacheConfig => ({
    ...
    monitoring: {
      ...
      metricsInterval: Number.parseInt(
        process.env.CACHE_METRICS_INTERVAL || '60000',
      ),
      alertThresholds: {
        hitRatio: Number.parseFloat(
          process.env.CACHE_HIT_RATIO_THRESHOLD || '0.8',
        ),
        responseTime: Number.parseInt(
          process.env.CACHE_RESPONSE_TIME_THRESHOLD || '100',
        ),
    ...
    warming: {
      ...
      strategies: (
        process.env.CACHE_WARMING_STRATEGIES || 'popular,recent'
      ).split(','),
