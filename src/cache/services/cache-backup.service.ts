...
export class CacheBackupService {
  ...
  private readonly backupDir =
    process.env.CACHE_BACKUP_DIR || './cache-backups';
  private redis: Redis;
  ...
  async listBackups(): Promise<string[]> {
    try {
      ...
      return files
        .filter(
          (file) => file.startsWith('cache-backup-') && file.endsWith('.json'),
        )
        .sort()
        .reverse();
    } catch (error) {
  ...
  private async cleanupOldBackups(): Promise<void> {
    try {
      ...
      const retention = Number.parseInt(
        process.env.CACHE_BACKUP_RETENTION || '7',
      );
