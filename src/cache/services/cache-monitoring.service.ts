...
export class CacheMonitoringService {
  ...
  private checkAlertThresholds(hitRatio: number, errorRate: number): void {
    if (hitRatio < 0.8) {
      this.logger.warn(
        `Cache hit ratio is low: ${(hitRatio * 100).toFixed(2)}%`,
      );
    }

    if (this.metrics.avgResponseTime > 100) {
      this.logger.warn(
        `Cache response time is high: ${this.metrics.avgResponseTime.toFixed(
          2,
        )}ms`,
      );
    }
    ...
    if (errorRate > 0.05) {
      this.logger.error(
        `Cache error rate is high: ${(errorRate * 100).toFixed(2)}%`,
      );
    }
  }
}
