...
export class CacheService {
  ...
  async get<T>(key: string, options: CacheOptions = {}): Promise<T | null> {
    ...
    try {
      ...
      if (
        this.config.layers.l1.enabled &&
        (options.layer === 'l1' || options.layer === 'both' || !options.layer)
      ) {
      ...
      if (
        this.config.layers.l2.enabled &&
        (options.layer === 'l2' || options.layer === 'both' || !options.layer)
      ) {
