...
describe('GuildsService', () => {
  ...
  beforeEach(async () => {
    ...
    guildMemberRepository = module.get<Repository<GuildMember>>(
      getRepositoryToken(GuildMember),
    );
    userRepository = module.get<Repository<User>>(getRepositoryToken(User));
    userPuzzleCompletionRepository = module.get<
      Repository<UserPuzzleCompletion>
  ...
  describe('createGuild', () => {
    it('should create a guild successfully', async () => {
      ...
      jest
        .spyOn(guildMemberRepository, 'create')
        .mockReturnValue(mockGuildMember as any);
      jest
        .spyOn(guildMemberRepository, 'save')
        .mockResolvedValue(mockGuildMember as any);
    ...
    it('should throw BadRequestException if user is already in a guild', async () => {
      ...
      jest
        .spyOn(guildMemberRepository, 'findOne')
        .mockResolvedValue(mockGuildMember as any);

      await expect(service.createGuild('user-1', dto)).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it('should throw BadRequestException if tag already exists', async () => {
      ...
      jest.spyOn(guildMemberRepository, 'findOne').mockResolvedValue(null);
      jest
        .spyOn(guildRepository, 'findOne')
        .mockResolvedValue(mockGuild as any);
  ...
  describe('joinGuild', () => {
    ...
    it('should throw BadRequestException if user is already in a guild', async () => {
      ...
      await expect(service.joinGuild('user-1', 'guild-1', dto)).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it('should throw NotFoundException if guild not found', async () => {
      ...
      await expect(service.joinGuild('user-2', 'guild-1', dto)).rejects.toThrow(
        NotFoundException,
      );
    });
  ...
  describe('leaveGuild', () => {
    ...
    it('should throw NotFoundException if user is not a member', async () => {
      ...
      await expect(service.leaveGuild('user-1', 'guild-1')).rejects.toThrow(
        NotFoundException,
      );
    });
  ...
  describe('kickMember', () => {
    it('should kick member successfully as officer', async () => {
      ...
      jest
        .spyOn(guildMemberRepository, 'findOne')
        .mockResolvedValueOnce(requesterMember as any);
      jest
        .spyOn(guildMemberRepository, 'findOne')
    ...
    it('should throw ForbiddenException if requester has insufficient role', async () => {
      ...
      await expect(
        service.kickMember('user-1', 'guild-1', 'user-2'),
  ...
  describe('updateMemberRole', () => {
    ...
    it('should throw ForbiddenException if requester is not owner', async () => {
      ...
      jest
        .spyOn(guildMemberRepository, 'findOne')
        .mockResolvedValue(requesterMember as any);

      await expect(
        service.updateMemberRole('user-1', 'guild-1', 'user-2', dto),
      ).rejects.toThrow(ForbiddenException);
    });
    ...
    it('should throw BadRequestException if trying to change own role', async () => {
      ...
      await expect(
        service.updateMemberRole('user-1', 'guild-1', 'user-1', dto),
  ...
  describe('getGuildProfile', () => {
    ...
    it('should throw NotFoundException if guild not found', async () => {
      ...
      await expect(service.getGuildProfile('guild-1')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('disbandGuild', () => {
    it('should disband guild successfully as owner', async () => {
      ...
      expect(guildMemberRepository.delete).toHaveBeenCalledWith({
        guildId: 'guild-1',
      });
    ...
    it('should throw NotFoundException if guild not found', async () => {
      ...
      await expect(service.disbandGuild('user-1', 'guild-1')).rejects.toThrow(
        NotFoundException,
      );
    });
  ...
  describe('recalculateGuildScore', () => {
    it('should recalculate score from member puzzle completions', async () => {
      const members = [
        mockGuildMember,
        { ...mockGuildMember, playerId: 'user-2' },
      ];

      jest
        .spyOn(guildMemberRepository, 'find')
        .mockResolvedValue(members as any);
      jest
        .spyOn(userPuzzleCompletionRepository, 'createQueryBuilder')
        .mockReturnValue({
          where: jest.fn().mockReturnThis(),
          leftJoin: jest.fn().mockReturnThis(),
          getMany: jest
            .fn()
            .mockResolvedValue([mockPuzzleCompletion, mockPuzzleCompletion]),
        } as any);
