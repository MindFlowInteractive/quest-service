...
export class QuestChainLeaderboardService {
  ...
  async getSpeedRunLeaderboard(
    ...
    const completedProgresses = await this.userProgressRepository
      ...
      .select(['progress.userId', 'progress.totalTime', 'progress.completedAt'])
  ...
  async getTotalCompletionsLeaderboard(
    ...
    const chainCompletions = await this.userProgressRepository
      .createQueryBuilder('progress')
      .select(['progress.userId', 'COUNT(progress.id) as completionCount'])
  ...
  async getUserRankInChain(
    userId: string,
    chainId: string,
  ): Promise<number | null> {
    const completedProgresses = await this.userProgressRepository
      .createQueryBuilder('progress')
      .select(['progress.userId', 'progress.totalTime'])
      .where('progress.questChainId = :chainId', { chainId })
      .andWhere('progress.status = :status', { status: 'completed' })
      .orderBy('progress.totalTime', 'ASC')
      .getRawMany();

    const userProgress = completedProgresses.find(
      (p) => p.progress_userId === userId,
    );
    if (!userProgress) return null;

    return (
      completedProgresses.findIndex((p) => p.progress_userId === userId) + 1
    );
  }
  ...
  async getChainCompletionStats(chainId: string): Promise<{
    ...
    const completedProgresses = allProgresses.filter(
    ...
    const successRate =
      totalParticipants > 0 ? (totalCompletions / totalParticipants) * 100 : 0;

    let averageTime = 0;
    let fastestCompletion = Infinity;

    if (completedProgresses.length > 0) {
      const totalTime = completedProgresses.reduce(
        (sum, progress) => sum + progress.totalTime,
        0,
      );
      averageTime = totalTime / completedProgresses.length;

      fastestCompletion = Math.min(
        ...completedProgresses.map((p) => p.totalTime),
      );
    } else {
      fastestCompletion = 0;
    }
