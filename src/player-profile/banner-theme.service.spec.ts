...
describe('BannerThemeService', () => {
  ...
  describe('getAllThemes', () => {
    it('should return all default themes', () => {
      ...
      expect(themes.some((theme) => theme.id === BannerTheme.COSMIC)).toBe(
        true,
      );
      expect(themes.some((theme) => theme.id === BannerTheme.FOREST)).toBe(
        true,
      );
    });
  });
  ...
  describe('getAvailableThemes', () => {
    it('should return only non-unlockable themes', () => {
      ...
      expect(themes.some((theme) => theme.id === BannerTheme.MINIMAL)).toBe(
        true,
      );
    });
  });
  ...
  describe('getUnlockableThemes', () => {
    it('should return only unlockable themes', () => {
      ...
      expect(themes.every((theme) => theme.isUnlockable)).toBe(true);
      expect(themes.some((theme) => theme.id === BannerTheme.SUNSET)).toBe(
        true,
      );
  ...
  describe('isThemeUnlocked', () => {
    ...
    it('should check unlock requirements for sunset theme', () => {
      ...
      expect(
        service.isThemeUnlocked(BannerTheme.SUNSET, userStatsUnlocked),
      ).toBe(true);
      expect(service.isThemeUnlocked(BannerTheme.SUNSET, userStatsLocked)).toBe(
        false,
      );
    });
    ...
    it('should check unlock requirements for neon theme', () => {
      ...
      expect(service.isThemeUnlocked(BannerTheme.NEON, userStatsUnlocked)).toBe(
        true,
      );
      expect(service.isThemeUnlocked(BannerTheme.NEON, userStatsLocked)).toBe(
        false,
      );
    });
    ...
    it('should check unlock requirements for gradient theme', () => {
      ...
      expect(
        service.isThemeUnlocked(BannerTheme.GRADIENT, userStatsUnlocked),
      ).toBe(true);
      expect(
        service.isThemeUnlocked(BannerTheme.GRADIENT, userStatsLocked),
      ).toBe(false);
    });

    it('should return false for unknown unlockable theme', () => {
      const userStats = {};
      const isUnlocked = service.isThemeUnlocked(
        'unknown-unlockable',
        userStats,
      );
  ...
  describe('getUserUnlockedThemes', () => {
    ...
    it('should return partial unlocked themes for intermediate user', () => {
      ...
      expect(
        unlockedThemes.some((theme) => theme.id === BannerTheme.SUNSET),
      ).toBe(true);
      expect(
        unlockedThemes.some((theme) => theme.id === BannerTheme.NEON),
      ).toBe(false);
    });
  });
});
