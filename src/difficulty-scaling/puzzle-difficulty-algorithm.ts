 ...
 */
export function calculatePuzzleDifficulty(
  puzzle: Puzzle | null,
  ...
  const completionRate =
    puzzle.analytics?.completionRate ??
    puzzle.completions / Math.max(1, puzzle.attempts);
  ...
  const voteScore =
    (votes.easy * 1 + votes.medium * 2 + votes.hard * 3 + votes.expert * 4) /
    totalVotes;
  // Normalize metrics
  ...
  const difficulty =
    1 + 4 * (0.4 * normCompletion + 0.3 * normTime + 0.3 * (voteScore / 4));
