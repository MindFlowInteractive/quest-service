...
export class SequenceGeneratorService {
  ...
  generateSequence(
    ...
    try {
      ...
      if (!pattern) {
        throw new Error(
          `No suitable pattern found for difficulty ${difficulty}`,
        );
      }
      ...
      const missingIndices = this.selectMissingIndices(
        length,
        missingCount,
        difficulty,
      );
  ...
  private selectPattern(
    ...
    const availablePatterns = Array.from(this.patterns.values()).filter(
      (pattern) => {
  ...
  private generateHints(
    ...
    switch (pattern.type) {
      ...
      case 'fibonacci':
        ...
        if (difficulty <= 3) {
          hints.push(
            `Start with the first two terms and add them to get the third`,
          );
        }
        break;
      ...
      case 'polynomial':
        hints.push(
          `The differences between consecutive terms follow a pattern`,
        );
    ...
    if (difficulty >= 5 && missingIndices.length > 0) {
      ...
      if (firstMissing > 0 && firstMissing < sequence.length - 1) {
        hints.push(
          `Focus on the relationship between positions ${
            firstMissing - 1
          }, ${firstMissing}, and ${firstMissing + 1}`,
        );
      }
    }
  ...
  private initializeDefaultPatterns(): void {
    ...
    this.registerPattern({
      ...
      validator: (sequence) => {
        ...
        for (let i = 2; i < sequence.length; i++) {
          if (
            Math.abs(sequence[i] - (sequence[i - 1] + sequence[i - 2])) > 0.001
          ) {
            return false;
          }
        }
