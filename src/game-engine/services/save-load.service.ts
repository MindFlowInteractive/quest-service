...
export class SaveLoadService {
  ...
  async saveGame(
    ...
    try {
      ...
      // Get active puzzles
      const activePuzzles = await this.stateManagement.getActivePuzzles(
        playerId,
      );
      ...
      const saveData: SaveGameData = {
        ...
        playerProgress: playerProgress
          ? this.serializePlayerProgress(playerProgress)
          : null,
        activePuzzles: activePuzzles.map((puzzle) =>
          this.serializePuzzleState(puzzle),
        ),
        currentSession: currentSession
          ? this.serializeSession(currentSession)
          : null,
        metadata: {
          includeHistory,
      ...
      if (includeHistory) {
        saveData.metadata.historicalPuzzles = await this.getHistoricalPuzzles(
          playerId,
        );
  ...
  async exportGameData(
    ...
    try {
      ...
      this.logger.error(
        `Error exporting game data for player ${playerId}:`,
        error,
      );
      throw error;
    }
  ...
  async importGameData(
    ...
    try {
      ...
      this.logger.error(
        `Error importing game data for player ${playerId}:`,
        error,
      );
      return {
        success: false,
  ...
  async createCheckpoint(
    ...
    try {
      ...
      this.logger.log(
        `Created checkpoint '${checkpointName}' for player ${playerId}`,
      );
    } catch (error) {
      this.logger.error(
        `Error creating checkpoint for player ${playerId}:`,
        error,
      );
      throw error;
    }
  ...
  async loadCheckpoint(
    ...
    try {
      ...
      const checkpointData =
        playerProgress.metadata.checkpoints[checkpointName].data;
      return await this.loadGame(playerId, checkpointData);
    } catch (error) {
      this.logger.error(
        `Error loading checkpoint '${checkpointName}' for player ${playerId}:`,
        error,
      );
      return {
        success: false,
  ...
  async listCheckpoints(
    ...
    try {
      ...
      return Object.entries(playerProgress.metadata.checkpoints).map(
      ...
      this.logger.error(
        `Error listing checkpoints for player ${playerId}:`,
        error,
      );
      return [];
    }
  ...
  async deleteCheckpoint(
    ...
    try {
      ...
      if (playerProgress?.metadata?.checkpoints?.[checkpointName]) {
        ...
        await this.playerProgressRepository.save(playerProgress);
        this.logger.log(
          `Deleted checkpoint '${checkpointName}' for player ${playerId}`,
        );
      }
    } catch (error) {
      this.logger.error(
        `Error deleting checkpoint '${checkpointName}' for player ${playerId}:`,
        error,
      );
      throw error;
    }
  ...
  private async migrateGameData(saveData: SaveGameData): Promise<SaveGameData> {
    this.logger.log(
      `Migrating game data from version ${saveData.version} to ${this.currentVersion}`,
    );
  ...
  private async getHistoricalPuzzles(
    playerId: string,
    limit = 50,
  ): Promise<any[]> {
