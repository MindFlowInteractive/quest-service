...
export class ValidationService implements IPuzzleValidator {
  ...
  async validateSolution(
    ...
    try {
      ...
      this.logger.error(
        `Solution validation error for puzzle ${puzzle.id}:`,
        error,
      );
      return {
        isValid: false,
  ...
  checkConstraints(puzzle: IPuzzle, state: any): boolean {
    try {
      ...
      this.logger.error(
        `Constraint check error for puzzle ${puzzle.id}:`,
        error,
      );
      return false;
    }
  ...
  private async defaultValidateMove(
    puzzle: IPuzzle,
    move: PuzzleMove,
  ): Promise<ValidationResult> {
  ...
  private async defaultValidateSolution(
    puzzle: IPuzzle,
    solution: any,
  ): Promise<ValidationResult> {
    const errors: ValidationError[] = [];
  ...
  private calculateMoveScore(puzzle: IPuzzle, move: PuzzleMove): number {
    ...
    if (
      ...
      (puzzle.maxMoves || Number.POSITIVE_INFINITY) * 0.5
    ) {
      score += 5;
    }
