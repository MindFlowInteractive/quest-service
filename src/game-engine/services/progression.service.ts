...
export class ProgressionService {
  ...
  async updateProgress(
    ...
    if (progress.totalPuzzlesSolved > 0) {
      progress.averageCompletionTime =
        progress.totalTimeSpent / progress.totalPuzzlesSolved;
    ...
    const skillAdjustment = this.calculateSkillAdjustment(
      performance,
      currentSkill,
    );
    progress.skillLevels[performance.puzzleType] = Math.max(
      1,
      currentSkill + skillAdjustment,
    );
    ...
    progress.statistics = {
      ...progress.statistics,
      lastPlayed: new Date(),
      puzzleTypeStats: this.updatePuzzleTypeStats(
        progress.statistics.puzzleTypeStats || {},
        performance,
      ),
    };
  ...
  registerUnlockCriteria(criteria: UnlockCriteria): void {
    ...
    this.logger.log(
      `Registered unlock criteria for puzzle: ${criteria.puzzleId}`,
    );
  }
  ...
  async getRecommendedDifficulty(
    ...
    let recommendedDifficulty = Math.min(
      Math.max(Math.round(skillLevel), 1),
      8,
    );
  ...
  async getProgressionPath(playerId: string): Promise<string[]> {
    ...
    for (const [puzzleId, criteria] of this.unlockCriteria) {
      if (!progress.unlockedPuzzles.includes(puzzleId)) {
        const missingRequirements = criteria.requiredPuzzles.filter(
          (req) => !progress.unlockedPuzzles.includes(req),
        );
    ...
    return availablePuzzles.sort((a, b) => {
      ...
      const skillDiffA = Math.abs(
        (criteriaA.minimumScore || 0) - progress.totalPuzzlesSolved,
      );
      const skillDiffB = Math.abs(
        (criteriaB.minimumScore || 0) - progress.totalPuzzlesSolved,
      );
  ...
  private meetsUnlockCriteria(
    ...
    if (
      ...
      progress.totalPuzzlesSolved < criteria.minimumScore
    ) {
      return false;
    }
    ...
    // Check success rate
    if (
      criteria.minimumSuccessRate &&
      progress.successRate < criteria.minimumSuccessRate
    ) {
      return false;
    }
    ...
    // Check streak
    if (
      criteria.requiredStreak &&
      progress.bestStreak < criteria.requiredStreak
    ) {
      return false;
    }
    ...
    if (criteria.requiredSkillLevel) {
      for (const [puzzleType, requiredLevel] of Object.entries(
        criteria.requiredSkillLevel,
      )) {
        const currentLevel =
          progress.skillLevels[puzzleType as PuzzleType] || 1;
  ...
  private calculateSuccessRate(
    ...
    const totalAttempts =
      progress.totalPuzzlesSolved + Math.max(0, -progress.currentStreak);
  ...
  private calculateSkillAdjustment(
    ...
    if (performance.completed) {
      ...
      const expectedTime = this.getExpectedTimeForDifficulty(
  ...
  private updatePuzzleTypeStats(
    ...
    typeStats.averageScore =
      (typeStats.averageScore * (typeStats.attempted - 1) + performance.score) /
      typeStats.attempted;
