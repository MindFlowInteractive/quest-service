...
export class StateManagementService {
  ...
  constructor(
    ...
    puzzleStateRepository: Repository<PuzzleState>,
  ) {
  ...
  async saveState(gameState: PuzzleGameState): Promise<void> {
    ...
    try {
      ...
      if (existingState) {
        await this.puzzleStateRepository.update(
          existingState.id,
          stateData as any,
        );
      } else {
      ...
      this.logger.error(
        `Failed to save state for puzzle ${gameState.puzzleId}:`,
        error,
      );
      throw error;
    }
  ...
  async deleteState(puzzleId: string, playerId: string): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        `Failed to delete state for puzzle ${puzzleId}:`,
        error,
      );
      throw error;
    }
  ...
  async getActivePuzzles(playerId: string): Promise<PuzzleGameState[]> {
    try {
      ...
      this.logger.error(
        `Failed to get active puzzles for player ${playerId}:`,
        error,
      );
      throw error;
    }
      ...
      return {
        ...
        completionRate:
          Number.parseInt(stats.total_puzzles) > 0
            ? (Number.parseInt(stats.completed_puzzles) /
                Number.parseInt(stats.total_puzzles)) *
              100
            : 0,
      };
    } catch (error) {
