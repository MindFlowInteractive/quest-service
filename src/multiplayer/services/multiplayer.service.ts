...
export class MultiplayerService {
  ...
  async createMultiplayerSession(
    ...
    this.logger.log(
      `Multiplayer session ${room.id} created with invite code ${inviteCode} by ${creator.id}`,
    );
    return room;
  }
  ...
  async joinSessionByCode(
    ...
    let room = await this.cacheManager.get<MultiplayerRoom>(
      `session:${inviteCode}`,
    );
    ...
    this.logger.log(
      `Player ${player.id} joined session ${room.id} via invite code ${inviteCode}`,
    );
    return room;
  }
  ...
  addToQueue(userId: string, skillLevel: number, type: RoomType) {
    if (!this.matchmakingQueue.find((q) => q.userId === userId)) {
      ...
      this.logger.log(
        `User ${userId} (level ${skillLevel}) added to ${type} matchmaking queue`,
      );
    }
  }
  ...
  findMatch() {
    ...
    for (let i = 0; i < this.matchmakingQueue.length - 1; i++) {
      ...
      if (Math.abs(p1.skillLevel - p2.skillLevel) <= 2 && p1.type === p2.type) {
        ...
        this.matchmakingQueue = this.matchmakingQueue.filter(
          (q) => !matchedPlayers.find((mp) => mp.userId === q.userId),
        );
  ...
  toggleSpectating(
    ...
    if (!spectatingAllowed) {
      room.spectators = [];
      this.logger.log(
        `Spectating disabled for room ${roomId}, all spectators removed`,
      );
    } else {
  ...
  isUserSpectating(roomId: string, userId: string): boolean {
    const room = this.rooms.get(roomId);
    return room
      ? !!room.spectators.find((s) => s.userId === userId && s.isActive)
      : false;
  }
}
