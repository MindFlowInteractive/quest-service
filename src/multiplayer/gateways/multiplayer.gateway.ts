  ...
  implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect
{
  ...
  async handleJoinSessionByCode(
    ...
    if (session) {
      ...
      this.server
        .to(session.id)
        .emit('playerJoinedSession', { player, session });
      this.server
        .to(`${session.id}-spectators`)
        .emit('playerJoinedSession', { player, session });
    ...
    return {
      event: 'error',
      data: 'Failed to join session - invalid code or session full',
    };
  }
  ...
  async handleReconnectToSession(
    ...
    const session = await this.multiplayerService.handlePlayerReconnection(
      data.roomId,
      data.userId,
    );
    ...
    if (session) {
      ...
      this.server
        .to(session.id)
        .emit('playerReconnected', { userId: data.userId });
      this.server
        .to(`${session.id}-spectators`)
        .emit('playerReconnected', { userId: data.userId });
    ...
    return {
      event: 'error',
      data: 'Failed to reconnect - grace period expired',
    };
  }
  ...
  handleCreateRoom(
    ...
    const room = this.multiplayerService.createRoom(
      data.type,
      player,
      data.settings,
    );
  ...
  handleJoinRoom(
    ...
    if (room) {
      ...
      this.server
        .to(`${data.roomId}-spectators`)
        .emit('playerJoined', { player, room });
  ...
  handleReady(
    ...
    const room = this.multiplayerService.setPlayerReady(
      data.roomId,
      data.userId,
      data.ready,
    );
    if (room) {
      ...
      if (room.status === RoomStatus.PLAYING) {
        ...
        this.server.to(`${data.roomId}-spectators`).emit('gameStarted', {
          startTime: room.startTime,
          puzzleId: room.puzzleId,
        });
      }
  ...
  handleUpdatePuzzleState(
    ...
    const room = this.multiplayerService.updatePuzzleState(
      data.roomId,
      data.state,
    );
    if (room) {
      ...
      this.server
        .to(`${data.roomId}-spectators`)
        .emit('puzzleStateUpdated', data.state);
    }
  }
  ...
  async handleSubmitSolution(
    @ConnectedSocket() client: Socket,
    @MessageBody()
    data: { roomId: string; userId: string; puzzleId: string; solution: any },
  ) {
    ...
    try {
      ...
      const result = await this.validationService.validateSolution(
        adaptedPuzzle,
        data.solution,
      );
      ...
      if (result.isValid) {
        ...
        if (room.type === RoomType.COLLABORATIVE) {
          ...
          const allPlayersSolved = room.players.every((p) =>
            p.solvedPuzzles.includes(data.puzzleId),
          );
          ...
          this.server
            .to(`${data.roomId}-spectators`)
            .emit('collaborativeSolutionVerified', {
              userId: data.userId,
              correct: true,
              score: result.score,
              totalScore: player?.score,
          ...
          if (allPlayersSolved) {
            ...
            // Update Redis
            await this.multiplayerService['cacheManager'].set(
            ...
            this.server
              .to(`${data.roomId}-spectators`)
              .emit('collaborativeSessionCompleted', {
                session: room,
                finalScores: room.players.map((p) => ({
                  userId: p.id,
                  username: p.username,
          ...
          this.server
            .to(`${data.roomId}-spectators`)
            .emit('competitiveSolutionVerified', {
              userId: data.userId,
              correct: true,
              score: result.score,
              totalScore: player?.score,
          ...
          const sessionDuration = room.startTime
            ? (new Date().getTime() - room.startTime.getTime()) / 1000 / 60
            : 0;
          if (
            sessionDuration >= room.settings.timeLimit ||
            player?.solvedPuzzles.length >= 10
          ) {
            room.status = RoomStatus.FINISHED;
            room.endTime = new Date();
            ...
            this.server
              .to(`${data.roomId}-spectators`)
              .emit('competitiveSessionCompleted', {
                session: room,
                winner: {
                  userId: winner.id,
                  username: winner.username,
  ...
  async handleUpdateCollaborativeState(
    ...
    this.server
      .to(`${data.roomId}-spectators`)
      .emit('collaborativeStateUpdated', {
        state: room.puzzleState,
        updatedBy: data.userId,
        timestamp: new Date(),
      });
  ...
  async handleSpectate(
    ...
    if (room) {
      ...
      this.server
        .to(`${data.roomId}-spectators`)
        .emit('spectatorJoined', { spectator });
  ...
  async handleLeaveSpectate(
    ...
    const room = this.multiplayerService.removeSpectator(
      data.roomId,
      data.userId,
    );
    if (room) {
      ...
      this.server
        .to(`${data.roomId}-spectators`)
        .emit('spectatorLeft', { userId: data.userId });
  ...
  async handleToggleSpectating(
    @ConnectedSocket() client: Socket,
    @MessageBody()
    data: { roomId: string; userId: string; spectatingAllowed: boolean },
  ) {
    const room = this.multiplayerService.toggleSpectating(
      data.roomId,
      data.spectatingAllowed,
    );
    if (room) {
      // Notify all room users
      this.server.to(data.roomId).emit('spectatingToggled', {
        spectatingAllowed: data.spectatingAllowed,
      });

      if (!data.spectatingAllowed) {
        // Remove all spectators from spectator room
        this.server
          .in(`${data.roomId}-spectators`)
          .socketsLeave(`${data.roomId}-spectators`);
      }

      return {
        event: 'spectatingToggled',
        data: { spectatingAllowed: data.spectatingAllowed },
      };
    }
