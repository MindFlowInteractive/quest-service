...
export class BlockchainEventsController {
  ...
  @ApiResponse({
    ...
    schema: {
      ...
      properties: {
        lastSyncedLedger: {
          type: 'number',
          description: 'Last processed ledger number',
        },
        eventsProcessedToday: {
          type: 'number',
          description: 'Number of events processed today',
        },
        errorCount: { type: 'number', description: 'Number of errors today' },
        registeredContracts: {
          type: 'array',
          items: { type: 'string' },
          description: 'Registered contract addresses',
        },
      },
    },
  })
  ...
  @ApiQuery({
    ...
    required: false,
    type: 'number',
    description: 'Ending ledger number (optional)',
  })
  @ApiResponse({
    status: 200,
    ...
    schema: {
      ...
      properties: {
        ...
        errors: {
          type: 'number',
          description: 'Number of errors during replay',
        },
      },
    },
  })
  async replayEvents(
    ...
    return await this.blockchainEventsService.replayEvents(
      fromLedger,
      toLedger,
    );
  }
