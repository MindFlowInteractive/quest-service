...
export class MatchmakingService {
  ...
  async getStatus(playerId: string) {
    ...
    const all = await this.repo.find({
      where: { puzzleType: entry.puzzleType, status: 'waiting' },
      order: { joinedAt: 'ASC' },
    });
  ...
  async pairPlayers() {
    ...
    for (let i = 0; i < waiting.length; i++) {
      const p1 = waiting[i];
      const window =
        50 + Math.floor((Date.now() - p1.joinedAt.getTime()) / 30000) * 25;
      const opponent = waiting.find(
        (p2) =>
          p2.playerId !== p1.playerId &&
          p2.puzzleType === p1.puzzleType &&
          Math.abs(p1.rating - p2.rating) <= window,
      );
      if (opponent) {
        // create GameSession
