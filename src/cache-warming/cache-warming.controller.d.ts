...
export declare class CacheWarmingController {
  ...
  preloadData(
    limit?: string,
  ): Promise<import('./entities/preload-data.entity').PreloadData[]>;
  upsertPreloadData(
    dto: CreatePreloadDataDto,
  ): Promise<import('./entities/preload-data.entity').PreloadData>;
  recordAccess(
    dto: RecordAccessDto,
  ): Promise<import('./entities/preload-data.entity').PreloadData>;
  ...
  warm(
    dto: WarmRequestDto,
  ): Promise<import('./cache-warming.service').WarmingResult>;
  invalidate(dto: ScheduleInvalidationDto): Promise<number>;
  scheduleInvalidation(
    dto: ScheduleInvalidationDto,
  ): Promise<import('./entities/cache-job.entity').CacheJob>;
