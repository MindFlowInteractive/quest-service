...
export class IntegrationsController {
  ...
  async linkAccount(@Req() req: any, @Body() dto: LinkSocialAccountDto) {
    const userId = req.user.sub || req.user.id;
    ...
    if (existing) {
      throw new BadRequestException(
        `${dto.provider} account is already linked`,
      );
    }
  ...
  async shareAchievement(@Req() req: any, @Body() dto: ShareContentDto) {
    ...
    if (dto.contentType !== 'achievement') {
      throw new BadRequestException(
        'Content type must be "achievement" for this endpoint',
      );
    }
  ...
  async shareLeaderboard(@Req() req: any, @Body() dto: ShareContentDto) {
    ...
    if (dto.contentType !== 'leaderboard') {
      throw new BadRequestException(
        'Content type must be "leaderboard" for this endpoint',
      );
    }
  ...
  private async processWebhookEvent(eventId: string): Promise<void> {
    ...
    try {
      ...
      this.logger.log(
        `Processing webhook event ${eventId}: ${event.source}/${event.eventType}`,
      );
