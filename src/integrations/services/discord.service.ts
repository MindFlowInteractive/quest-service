...
export class DiscordService {
  ...
  constructor(private readonly configService: ConfigService) {
    this.botName =
      this.configService.get<string>('DISCORD_BOT_NAME') || 'Quest Service Bot';
    this.defaultWebhookUrl = this.configService.get<string>(
      'DISCORD_WEBHOOK_URL',
    );
  }
  ...
  async postAchievement(
    ...
    const embed: DiscordEmbed = {
      ...
      color: 0xffd700, // Gold
      fields: [{ name: 'Player', value: achievement.userId, inline: true }],
  ...
  verifyWebhookSignature(
    ...
    if (!publicKey) {
      this.logger.warn(
        'Discord public key not configured — skipping verification',
      );
      return false;
    }
  ...
  async sendWebhook(
    ...
    try {
      ...
      return {
        success: false,
        message: `Discord webhook error: ${error.message}`,
      };
    }
  }
}
