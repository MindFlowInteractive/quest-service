...
describe('IntegrationNotificationService', () => {
  ...
  beforeEach(async () => {
    settingsRepo = {
      ...
      save: jest.fn((entity) =>
        Promise.resolve({ id: 'settings-1', ...entity }),
      ),
    };
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        },
        {
          ...
          useValue: {
            postAchievement: jest
              .fn()
              .mockResolvedValue({ success: true, message: 'Sent' }),
            postLeaderboardUpdate: jest
              .fn()
              .mockResolvedValue({ success: true, message: 'Sent' }),
          },
        },
        {
          ...
          useValue: {
            shareAchievement: jest.fn().mockReturnValue({
              success: true,
              shareUrl: 'https://twitter.com/intent/tweet?text=test',
              message: 'Generated',
            }),
            shareLeaderboardRank: jest.fn().mockReturnValue({
              success: true,
              shareUrl: 'https://twitter.com/intent/tweet?text=test',
              message: 'Generated',
            }),
            postTweet: jest
              .fn()
              .mockResolvedValue({ success: true, message: 'Posted' }),
          },
        },
      ],
    }).compile();

    service = module.get<IntegrationNotificationService>(
      IntegrationNotificationService,
    );
  ...
  describe('notifyLeaderboardRank', () => {
    ...
    it('should notify Discord for leaderboard rank', async () => {
      ...
      const result = await service.notifyLeaderboardRank(
        'user-1',
        leaderboardData,
      );
