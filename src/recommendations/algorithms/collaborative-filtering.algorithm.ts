...
export class CollaborativeFilteringAlgorithm {
  ...
  async generateRecommendations(
    ...
    const userInteractions = await this.userInteractionRepo.findUserCompletions(
      userId,
    );
  ...
  private async findSimilarUsers(
    ...
    const similarUserData =
      await this.userInteractionRepo.findSimilarUserInteractions(
        userPuzzleIds,
        userId,
  ...
  private async getRecommendationsFromSimilarUsers(
    ...
    for (const similarUser of similarUsers) {
      const similarUserInteractions =
        await this.userInteractionRepo.findUserCompletions(
          similarUser.userId,
          50,
        );
    ...
    for (const [puzzleId, candidate] of candidatePuzzles) {
      if (
        ...
        candidate.contributingUsers.length >= 2
      ) {
        ...
        recommendations.push({
          ...
          reason: `Recommended by ${
            candidate.contributingUsers.length
          } similar users with ${(candidate.totalWeight * 100).toFixed(
            0,
          )}% confidence`,
