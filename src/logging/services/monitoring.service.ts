...
export class MonitoringService {
  ...
  async getSystemHealth(): Promise<SystemHealth> {
    ...
    if (
      hasUnhealthy ||
      errorRate > this.config.alerting.rules.errorRate.threshold
    ) {
      status = 'unhealthy';
    } else if (
      hasDegraded ||
      avgResponseTime > this.config.alerting.rules.responseTime.threshold
    ) {
      status = 'degraded';
    }
  ...
  private async checkAlertingRules(health: SystemHealth): Promise<void> {
    ...
    // Check error rate
    if (
      health.metrics.errorRate > this.config.alerting.rules.errorRate.threshold
    ) {
      alerts.push(
    ...
    if (
      ...
      this.config.alerting.rules.responseTime.threshold
    ) {
      alerts.push(
        `High response time: ${health.metrics.avgResponseTime.toFixed(2)}ms`,
      );
    }
    ...
    if (
      ...
      this.config.alerting.rules.memoryUsage.threshold
    ) {
      alerts.push(
        `High memory usage: ${(health.memory.percentage * 100).toFixed(2)}%`,
      );
    }
      ...
      if (serviceHealth.status === 'unhealthy') {
        alerts.push(
          `Service ${serviceName} is unhealthy: ${
            serviceHealth.error || 'Unknown error'
          }`,
        );
      }
    }
