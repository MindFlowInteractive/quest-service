...
export class AlertingService {
  ...
  constructor(config: any) {
    ...
    if (this.config.alerting.channels.email.enabled) {
      this.emailTransporter = nodemailer.createTransport(
        this.config.alerting.channels.email.smtp,
      );
    }
  }
  ...
  private async sendEmailAlert(alert: Alert): Promise<void> {
    if (!this.config.alerting.channels.email.enabled || !this.emailTransporter)
      return;
  ...
  private async sendSlackAlert(alert: Alert): Promise<void> {
    ...
    try {
      ...
      const response = await fetch(
        this.config.alerting.channels.slack.webhookUrl,
        {
          method: 'POST',
        ...
        <body style="font-family: Arial, sans-serif; margin: 0; padding: 20px;">
          <div style="max-width: 600px; margin: 0 auto;">
            ...
            <div style="background-color: #f9f9f9; padding: 20px; border-radius: 0 0 5px 5px;">
              ...
              ${
                alert.metadata
                  ? `<p><strong>Metadata:</strong> <pre>${JSON.stringify(
                      alert.metadata,
                      null,
                      2,
                    )}</pre></p>`
